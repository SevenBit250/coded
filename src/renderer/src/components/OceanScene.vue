 ...
 * across water. Purely ambient — no pointer interaction. Canvas code is
 * ported verbatim; only the framework wrapper changed.
 */
...
function mount(): void {
  ...
  const sprites = (props.dark ? DARK_SHADES : LIGHT_SHADES).map(makeSprite)
...
<template>
  <div class="ocean-scene" aria-hidden="true">
    ...
    <img class="startup-shark" src="/shark-icon.svg" alt="" />
    <canvas ref="canvasEl" class="ocean-canvas" />
  </div>
</template>
